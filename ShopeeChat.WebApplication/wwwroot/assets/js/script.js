...
(function ($) {
    ...
    //setInterval(typingMessage, 10000);
...
function typingMessage() {
    $.ajax({
        type: "GET",
        url: "/Conversations/GetNewMessage",
        cache: false,
        success: function (response) {
            console.log(response);
            var avatarCustomer = $('#avatarCustomer').val();
            var nameCustomer = $('#nameCustomer').val();
            var toId = $('#toId').val();
            response.forEach(function (entry) {
                if (entry.toId == toId && response.length > 0 && entry.latestMessageFromId != 20878264) {
                    $('<li class="sent last typing-m"> <div class="media"><div class="profile mr-4 bg-size" style="background-image: url(&quot;' + avatarCustomer + '&quot;); background-size: cover; background-position: center center; display: block;"><img class="bg-img"' + 'src="' + avatarCustomer + '" alt="Avatar" style="display: none;"></div><div class="media-body"><div class= "contact-name"><h5>' + nameCustomer + '</h5><h6>01:42 AM</h6><ul class= "msg-box" ><li><h5><div class="type"><div class="typing-loader"></div></div></h5></li></ul ></div ></div ></div ></li >')
                        .appendTo($('.messages .chatappend'));
                    $(".messages").animate({ scrollTop: 9999 }, "fast");
                    setTimeout(function () {
                        $('.typing-m').hide();
                        $('<li class="sent"> <div class="media"> <div class="profile mr-4 bg-size" style="background-image: url(&quot;' + avatarCustomer + '&quot;); background-size: cover; background-position: center center; display: block;"></div><div class="media-body"> <div class="contact-name"> <h5>' + nameCustomer + '</h5> <h6>01:35 AM</h6> <ul class="msg-box"> <li> <h5>' + entry.latestMessageContent.text + '</h5> <div class="badge badge-success sm ml-2"> R</div></li></ul> </div></div></div></li>')
                            .appendTo($('.messages .chatappend'));
                        $(".messages").animate({ scrollTop: 9999 }, "fast");
                    }, 2000);
                }
